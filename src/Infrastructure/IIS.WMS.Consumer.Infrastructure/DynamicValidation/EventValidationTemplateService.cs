    ...
    ILogger<EventValidationTemplateService> logger) : IEventValidationTemplateService
{
    ...
    {
        var blobName = ToBlobName(transport, identifier);
        ...
        return new EventValidationTemplateResponse(transport, identifier, code);
    }
    ...
    {
        // Trailing slash so "Kafka" doesn't also match a "KafkaLegacy/" folder.
        var prefix = string.IsNullOrEmpty(transport) ? null : $"{transport}/";
    ...
    {
        var blobName = ToBlobName(request.Transport, request.Identifier);
        ...
        return new EventValidationTemplateResponse(request.Transport, request.Identifier, request.Code);
    }
        ...
        string transport, string identifier, UpdateEventValidationTemplateRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return new EventValidationTemplateResponse(transport, identifier, request.Code);
    }
    ...
    /// <summary>The <c>{transport}/{identifier}.cs</c> blob path a template is stored under.</summary>
    private static string ToBlobName(string transport, string identifier) =>
        $"{transport}/{identifier}{TemplateExtension}";

    /// <summary>
    /// Parses a container blob name back into a template identity, or <see langword="null"/> for a
    /// blob that doesn't follow the <c>{transport}/{identifier}.cs</c> convention (anything placed in
    ...
    private static EventValidationTemplateSummary? TryParseBlobName(string blobName)
    {
        ...
        return segments is [{ Length: > 0 } transport, { Length: > 0 } identifier]
            ? new EventValidationTemplateSummary(transport, identifier)
            : null;
    }
}
