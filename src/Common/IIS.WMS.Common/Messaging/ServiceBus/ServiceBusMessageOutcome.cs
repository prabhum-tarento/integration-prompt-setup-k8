...
/// <c>ProcessSessionMessageEventArgs</c>/<c>ProcessMessageEventArgs</c>). A Service Bus consumer hosted
/// service splits its real event handler into a thin adapter (still wired to the real SDK event,
/// settles the message per this outcome) and an internal <c>HandleMessageAsync(ServiceBusReceivedMessage,
