...
/// <summary>
/// CRUD over the event validation templates stored as <c>{Transport}/{Identifier}.cs</c> blobs in the
/// hot-tier validation-template container. Each template holds a C# script the owning transport's
/// consumer executes against every matching message right after its own <c>ValidateAsync</c> - so
/// create/update compile the code first and reject anything that doesn't build, rather than letting a
/// broken script dead-letter live traffic. Implemented in Infrastructure (blob-backed), same as
/// <c>IFileStore</c>.
/// </summary>
public interface IEventValidationTemplateService
{
    ...
    Task<EventValidationTemplateResponse?> GetAsync(
        string transport, string identifier, CancellationToken cancellationToken = default);

    /// <summary>Lists every stored template's identity, optionally narrowed to one transport.</summary>
    /// <param name="transport">Transport to filter on, or <see langword="null"/> for every template.</param>
    ...
    Task<IReadOnlyList<EventValidationTemplateSummary>> ListAsync(
        string? transport = null, CancellationToken cancellationToken = default);
    ...
    /// <exception cref="IIS.WMS.Common.DynamicValidation.TemplateCompilationException">The code doesn't compile - nothing is stored.</exception>
    ...
    /// <returns>The stored template.</returns>
    /// <exception cref="IIS.WMS.Common.DynamicValidation.TemplateCompilationException">The code doesn't compile - the stored template is left unchanged.</exception>
    ...
    Task<EventValidationTemplateResponse> UpdateAsync(
        string transport, string identifier, UpdateEventValidationTemplateRequest request, CancellationToken cancellationToken = default);
    ...
    /// (<c>x</c>, <c>header</c>, <c>_log</c>, plus the <c>TryGetHeader</c>/<c>WellKnownHeaderNames</c>
