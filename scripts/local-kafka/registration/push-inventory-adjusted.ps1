...
falls back to the InventoryAdjusted sample matching
registration/events/inventory-events/inventory.InventoryAdjusted/schema.avsc (see that
folder's 1.json for the reference shape this default is built from).
#>
param(
    ...
    # Schema Registry subject to resolve schema_id from - the actual TopicRecordNameStrategy
    # subject registered for this event on the shared "inventory-events" topic (confirmed via
    # register-defaults.bat's phase-3 logic), not a plain "$Topic-value" suffix.
    [string]$SchemaName = 'inventory-events-net.pandora.nexus.event.inventory.InventoryAdjusted',
    ...
    # printed or compact). Defaults to the InventoryAdjusted sample below if omitted.
    [string]$Body,
    ...
    [string]$EventType = 'inventory.InventoryAdjusted',
...
# Shape matches net.pandora.nexus.event.inventory.InventoryAdjusted (see
# registration/events/inventory-events/inventory.InventoryAdjusted/schema.avsc) - a channel,
# and one adjustment record with a location and an array of adjustmentLines. Only used when
# the caller doesn't pass -Body. adjustmentDate must be epoch milliseconds (a JSON number),
# not an ISO-8601 string - the avsc declares it as {"type": "long", "logicalType":
# "timestamp-millis"} and the REST Proxy 400s with "Expected long. Got VALUE_STRING"
# otherwise (same failure mode as push-inventory-state-changed.ps1's own changeDate).
if (-not $Body) {
    $adjustmentDateMillis = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
    $Body = @"
{"channel":"OTHER_STORES","adjustment":{"referenceId":"2519503_163800C01-60","adjustmentDate":$adjustmentDateMillis,"entity":null,"type":"UNKNOWN","state":{"state":"AVAILABLE","status":"PICKABLE"},"location":{"id":"CAECOM","type":"THIRD_PARTY_LOGISTICS"},"reason":"ADJUSTMENT","adjustmentLines":[{"lineNum":"1","productId":"163800C01-60","itemName":null,"quantity":-1,"units":null,"countryOfOrigin":"UNKNOWN","hallmarking":"NON","netWeight":null,"tareWeight":null,"unitPrice":null,"commodityCode":null,"itemCategoryLocalized":null,"itemMaterialNameLocalized":null,"inventoryRegistrationId":null,"customsRegistrationLineNum":null,"isBonded":null}]}}
"@
}
...
try {
    ...
    # snake_case throughout, including the top-level ProduceRequest's own partition_id.
    #
